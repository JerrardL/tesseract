...
- Captioning, Image Clasification (Keras and Image AI), Video Object Recognition and Facial Expression now include some Tensorflow configuration for GPU memory and use a different base docker image, provided by [NVIDIA NGC](https://ngc.nvidia.com/catalog/containers). The respective `requirements.txt` files have been altered to remove tensorflow as it is included in the docker image.
